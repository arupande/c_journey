    ...
    return new Promise((resolve, reject) => {
        ...
        MongoPool.getInstance(async function (client) {
            ...
            let cursor = collection.aggregate([
                            ...
                            "indvFreq": {
                                "$push": {
                                    "$cond":[
                                        { $not: ["$indv.loanDetail.noEMI"] },
                                        null,
                                        {"$ifNull":["$indv.loanDetail.freq","NA"]}
                                    ]
                                }
                            }
                        }
                    ...
                    {
                        "$group":{
                            "_id":{_id:"$_id","resInstalment":"$resInstalment","indvInstalment":"$indvInstalment","indvFreq":"$indvFreq",},
                            "grpInstalment": {
                                "$push": {
                                    "$cond":[
            ...
            let cursor = collection.aggregate([
                ...
                },
                {
                    "$project":{
                        ...
                        ZeroBalanceAccounts:{"$sum":[{"$size":{ $ifNull: [ "$resZeroBal", [] ] }},{"$size":{ $ifNull: [ "$indvZeroBal", [] ] }},{"$size":{ $ifNull: [ "$grpZeroBal", [] ] }}]},
                        NumberOfAccounts:"$NumberOfAccounts",
                        NumberOfEnquiries:"$NumberOfEnquiries",
                        resDisbursedAmt:"$resDisbursedAmt",
                        indvDisbursedAmt:"$indvDisbursedAmt",
...
function getEMIByfreq(freq, instalment) {
    if(freq === "biweekly"){
        instalment = parseFloat(instalment.replace(/,/g,'')) * 2
    }else if(freq === "weekly"){
        instalment = parseFloat(instalment.replace(/,/g,'')) * 4
    }else if(freq === "quarterly"){
        instalment = parseFloat(instalment.replace(/,/g,'')) / 3
    }else {
        instalment = parseFloat(instalment.replace(/,/g,''))
    }
    return instalment;
}
                    ...
                    item.forEach(function(entry){
                        ...
                        if(crif[pre+entry+"CombinedPaymentHistory"] !== undefined ) {
                            for(let i = 0; i < crif[pre+entry+"CombinedPaymentHistory"].length ; i++){
                                ...
                                let emi = 0
                                if(!acctType.toLowerCase().includes("gold")){
                                ...
                                for(let j = 0;j<combinedPaymentHistory.length;j++){
                                    ...
                                    if (combinedPaymentHistoryStr !== "") {
                                        if(crif[pre+entry+"DPDList"][i]){
                                            let dpdEntry = crif[pre+entry+"DPDList"][l][j];
                                            if(dpdEntry > 0 ){
                    ...
                    item.forEach(function (entry){
                        ...
                        if(disbursedAmtArr !== undefined ) {
                            for(let i = 0; i < disbursedAmtArr.length; i++){
                                if(statusArr[i].toUpperCase() === "ACTIVE" && currentBalList[i] > 100){//new code
